...
btNode* rotateRight(btNode* root) {
	 btNode* btTemp = (*(root->vArray + 0))->left;
...
btNode* rotateLeft(btNode* root) {
	...
	root->vArray = growNodeArray(root->vArray, ++root->vIndex);
	*(root->vArray + root->vIndex) = *(btTemp->vArray + btTemp->vIndex);
	if( (*(root->vArray + root->vIndex))->left != NULL ) {
		(*(root->vArray + (root->vIndex-1)))->right = (*(root->vArray + root->vIndex))->left;
		(*(root->vArray + (root->vIndex-1)))->rHeight = (*(((*(root->vArray + (root->vIndex-1)))->right)->vArray + 0))->rHeight + 1;
	}
	free(btTemp->vArray);
	free(btTemp);
	return root;
